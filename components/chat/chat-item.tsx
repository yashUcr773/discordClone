...
const roleIconMap = {
    ...
    [MemberRole.MODERATOR]: <ShieldCheck className="w-4 h-4 ml-2 text-indigo-500"></ShieldCheck>,
    [MemberRole.ADMIN]: <ShieldAlert className="w-4 h-4 ml-2 text-rose-500"></ShieldAlert>,
                ...
                <div className="flex flex-col w-full">
                    ...
                    {isPdf && (
                        <div className='relative flex items-center p-2 mt-2 rounded-md bg-background/10'>
                            <FileIcon className='h-10 2-10 fill-indigo-200 stroke-indigo-400'></FileIcon>
                                    ...
                                    <FormItem className="flex-1">
                                        <FormControl>
                                            <div className="relative w-full">
                                                <Input disabled={isLoading} className="p-2 bg-zinc-200/90 dark:bg-zinc-700/75 border-none border-0 focus-visible:ring-0 focus-visible:ring-offset-0 text-zinc-600 dark:text-zinc-200" placeholder="Edited Message" {...field}></Input>
                                            </div>
                                        </FormControl>
                                    </FormItem>
                                )}></FormField>
                ...
                <div className="hidden group-hover:flex items-center gap-x-2 absolute p-1 -top-2 right-5 bg-white dark:bg-zinc-800 border rounded-sm">
                    {canEditMessage && (
                        <ActionTooltip label="Edit">
                            <Edit onClick={() => { setIsEditing(true) }} className="cursor-pointer ml-auto w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition"></Edit>
                        </ActionTooltip>
                    )}
                    <ActionTooltip label="Delete">
                        <Trash onClick={() => { onOpen('deleteMessage', { apiUrl: `${socketUrl}/${id}`, query: socketQuery }) }} className="cursor-pointer ml-auto w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition"></Trash>
                    </ActionTooltip>
                </div>
            )}
        </div>
